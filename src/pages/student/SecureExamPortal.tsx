...
export default function SecureExamPortal({ presetExamCode, onBackToTeacher }: SecureExamPortalProps) {
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-white/3 text-slate-900 flex flex-col">
      <header className="border-b border-[var(--color-glass-light-stroke)] bg-white">
        <div className="max-w-5xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-sm font-black text-[var(--color-text-primary)]">درگاه امن آزمون (آلفا نسخه ۲)</h1>
              <p className="text-[11px] text-[var(--color-text-tertiary)]">مجهز به ذخیره‌ساز هوشمند آفلاین در قطعی اینترنت</p>
            </div>
          </div>
          <button onClick={onBackToTeacher} className="text-xs font-bold text-[var(--color-accent)] hover:underline">
            بازگشت به پنل معلم
          </button>
        </div>
      </header>
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-4 py-8">
        {error && (
          <div className="mb-5 bg-rose-50 border border-rose-200 text-[var(--color-danger)]/80 rounded-2xl p-4 text-xs font-bold flex items-start gap-2">
        ...
        {phase === 'login' && (
          <form onSubmit={handleStartSession} className="bg-white rounded-3xl border border-[var(--color-glass-light-stroke)] shadow-sm p-6 md:p-8 max-w-xl mx-auto space-y-5">
            <div className="text-center space-y-2">
              ...
              <p className="text-xs text-[var(--color-text-tertiary)] leading-relaxed">کد ملی در مرورگر ذخیره نمی‌شود؛ اعتبارسنجی و تطبیق آن فقط در بک‌اند انجام می‌شود.</p>
            </div>
            <label className="block space-y-1.5 text-right">
              <span className="text-xs font-bold text-[var(--color-text-secondary)]">کد آزمون</span>
              <input value={examCode} onChange={(e) => setExamCode(e.target.value.toUpperCase())} className="w-full rounded-xl border border-[var(--color-glass-light-stroke)] bg-white/3 px-4 py-3 text-sm font-bold text-left ltr" />
            </label>
            <label className="block space-y-1.5 text-right">
              <span className="text-xs font-bold text-[var(--color-text-secondary)]">کد ملی دانش‌آموز</span>
              <input value={nationalId} onChange={(e) => setNationalId(e.target.value.replace(/\D/g, '').slice(0, 10))} placeholder="مثلاً 1000000001" className="w-full rounded-xl border border-[var(--color-glass-light-stroke)] bg-white/3 px-4 py-3 text-sm font-bold text-left tracking-widest" />
            </label>
            <button disabled={loading || nationalId.length !== 10} className="w-full rounded-xl bg-[var(--color-accent)] disabled:bg-indigo-300 text-white py-3 text-sm font-black flex items-center justify-center gap-2">
        ...
        {phase === 'ready' && exam && student && (
          <div className="bg-white rounded-3xl border border-[var(--color-glass-light-stroke)] shadow-sm p-6 md:p-8 max-w-2xl mx-auto space-y-5 text-center">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
              <div className="bg-white/3 rounded-2xl p-3"><span className="block text-[var(--color-text-tertiary)]">آزمون</span><strong>{exam.title}</strong></div>
              <div className="bg-white/3 rounded-2xl p-3"><span className="block text-[var(--color-text-tertiary)]">درس</span><strong>{exam.subject}</strong></div>
              <div className="bg-white/3 rounded-2xl p-3"><span className="block text-[var(--color-text-tertiary)]">زمان</span><strong>{toPersianDigits(exam.durationMinutes)} دقیقه</strong></div>
            </div>
            <button onClick={handleLoadPayload} disabled={loading} className="rounded-xl bg-[var(--color-accent)] disabled:bg-indigo-300 text-white px-6 py-3 text-sm font-black inline-flex items-center gap-2">
          ...
          <div className="space-y-5">
            <div className="bg-white rounded-3xl border border-[var(--color-glass-light-stroke)] p-5 flex flex-col md:flex-row md:items-center justify-between gap-3 sticky top-0 z-10 shadow-xs">
              <div>
                ...
                <p className="text-xs text-[var(--color-text-tertiary)] mt-1">پاسخ‌داده‌شده: {toPersianDigits(answeredCount)} از {toPersianDigits(questions.length)}</p>
              </div>
              <div className="flex flex-wrap items-center gap-3 text-xs">
                {queuedCount > 0 ? (
                  <div className="bg-[var(--color-warning-soft)] border border-[var(--color-warning)]/20 text-[var(--color-warning)]/80 rounded-xl px-3 py-2 flex items-center gap-2">
                    ...
                    <button onClick={handleSyncQueued} disabled={syncing} className="bg-amber-200 hover:bg-amber-300 px-2 py-1 rounded-lg font-black text-[var(--color-warning)] inline-flex items-center gap-1 cursor-pointer transition-colors">
                      {syncing ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <RefreshCw className="w-3.5 h-3.5" />}
                      همگام‌سازی الآن
                    </button>
                  </div>
                ) : (
                  <div className="text-[var(--color-text-tertiary)] flex items-center gap-1.5" title="اتصال زنده برقرار است">
                    <Wifi className="w-4 h-4 text-emerald-500" />
                    <span className="text-[11px]">ارتباط آنلاین</span>
                  </div>
                )}
                {saving && <span className="text-[var(--color-warning)] font-bold">در حال ذخیره...</span>}
                {savedAt && !saving && <span className="text-emerald-600 font-bold max-w-xs truncate">{savedAt}</span>}
                <button onClick={handleSubmit} disabled={loading} className="rounded-xl bg-[var(--color-success)] disabled:bg-[var(--color-success)] text-white px-5 py-2.5 font-black inline-flex items-center gap-2 shadow-sm hover:bg-[var(--color-success)]/90 cursor-pointer">
            ...
            {questions.map((question, index) => (
              <section key={question.id} className="bg-white rounded-3xl border border-[var(--color-glass-light-stroke)] shadow-sm p-5 md:p-6 space-y-4">
                <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--color-glass-light-stroke)] pb-3">
                  <h3 className="font-black text-[var(--color-text-primary)] text-sm">سوال {toPersianDigits(index + 1)}: {question.title}</h3>
                  <span className="bg-[var(--color-warning-soft)] text-[var(--color-warning)] border border-[var(--color-warning)]/10 rounded-xl px-3 py-1 text-xs font-black">{toPersianDigits(question.points)} نمره</span>
                </div>
                <p className="text-sm text-[var(--color-text-secondary)] leading-relaxed">{question.body?.text || 'متن سوال موجود نیست.'}</p>
                {(question.body as any)?.imageUrl && (
                  <div className="my-3">
                    <img src={(question.body as any).imageUrl} alt="پیوست سوال" className="max-h-64 rounded-2xl object-cover border border-[var(--color-glass-light-stroke)] shadow-xs" />
                  </div>
                )}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {(question.body?.options || []).map((option) => {
                    ...
                    return (
                      <button key={option.id} onClick={() => handleSaveAnswer(question.id, option.id)} className={(selected ? 'border-[var(--color-accent)]/100 bg-[var(--color-accent-soft)] text-indigo-800 shadow-xs font-black ' : 'border-[var(--color-glass-light-stroke)] bg-white/3 text-[var(--color-text-secondary)] hover:border-[var(--color-glass-light-stroke)] ') + 'rounded-2xl border p-4 text-right text-xs transition-all flex items-center justify-between cursor-pointer'}>
                        <span>{option.text}</span>
                        {option.imageUrl && <img src={option.imageUrl} alt="گزینه" className="w-10 h-10 rounded-lg object-cover border border-[var(--color-glass-light-stroke)] ml-2" />}
                        <span className="font-mono text-[10px] text-[var(--color-text-tertiary)] font-bold bg-white px-2 py-0.5 rounded-md border">{option.id}</span>
                      </button>
                    );
                  })}
                </div>
        ...
        {phase === 'submitted' && (
          <div className="bg-white rounded-3xl border border-[var(--color-glass-light-stroke)] shadow-sm p-8 max-w-xl mx-auto text-center space-y-4 animate-in zoom-in-95 duration-200">
            ...
            <h2 className="font-black text-[var(--color-text-primary)] text-lg">پاسخ شما با موفقیت در سامانه ثبت نهایی شد.</h2>
            <p className="text-xs text-[var(--color-text-tertiary)] leading-relaxed">پس از ارسال نهایی، نشست آزمون قفل شده و امکان تغییر پاسخ‌ها وجود ندارد. برای مشاهده کارنامه یا نتایج به دستورالعمل دبیر خود مراجعه نمایید.</p>
          </div>
        )}
      </main>
    </div>
